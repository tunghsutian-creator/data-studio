...
7. 完整 Windows 路线按“先验证最高风险假设，再产品化”的顺序执行：Phase 0 基线完成后，先验证本地 AI 可行性，再完成 AI 产品闭环、统一导出、Obsidian、Bridge 和 Web 发布。
...
- GitHub 只保存源码、文档、小型测试代码和经批准的 UI 资产；raw、catalog、Inbox、Vault、模型、运行时、备份、导出物及由真实数据派生的样本不得进入 Git。提交和推送必须通过仓库数据安全检查。
...
- 成功不是“模型能够启动”，而是减少人工整理负担，同时保持零危险文件写入和完整审计链。
- 确定性解析与规则始终先于 AI；AI 只处理 Unknown、低置信、冲突、像素证据和用户主动重分析。
- 先验证显存、延迟、准确率和多轮稳定性，再建设持久队列和产品 UI，避免为不可用模型建设整套基础设施。
- 任一 AI 闸门失败都不得阻塞 rules-only、统一导出和其余 Windows 产品能力。

### Phase 1：Windows 本地 AI 可行性闸门

内容：

- 建立独立、只读的 benchmark harness；不修改正式 catalog，不向模型提供会泄漏答案的父目录名。
- 使用便携、固定 commit/build 的 llama.cpp CUDA runtime；不做全局系统安装，只监听 `127.0.0.1`。
- 使用官方 `Qwen3-VL-8B-Instruct Q8_0` 主 GGUF 与 Q8_0 mmproj；记录精确来源 revision、许可证、字节数与 SHA-256。
- 首轮诊断集约 30 个：全部 Unknown、低置信样本，以及覆盖 SEM/TIFF、表格和文本的分层代表样本。
- 输入证据有明确上限；大型 TIFF 使用受控全图预览和必要标注区域，不把任意 raw 路径或无限大小内容交给模型。
- 测量结构化输出、准确率、p50/p95、显存峰值、连续视觉请求状态污染和错误恢复。

退出条件：

- Q8 在 RTX 5080 16 GB 上以 context 8192、单并发稳定运行，峰值 VRAM 不超过 90%。
- 诊断任务 p95 不超过 30 秒；连续请求无崩溃、乱码、跨请求状态污染或显存持续增长。
- 后端能够拒绝无效 JSON、非法枚举和无证据建议，且 benchmark 不对 raw、catalog 或 Git 产生写入。
- 若 Q8 不通过，停止并提交报告；只有用户批准后才下载或测试 Q4，不静默降级。

### Phase 2：AI 最小产品闭环

内容：

- 实现 `LocalModelProvider` 契约、fake provider 测试和 llama.cpp provider。
- 增加 `model_registry`、`ai_tasks`、`ai_runs` migration；任务状态持久化、输入指纹幂等、超时、重试和 abstain。
- 建立 deterministic evidence builder：扩展名、文件头、sheet/列、仪器元数据、受限文本和图像预览。
- 模型只输出版本化 JSON Schema；后端使用严格 schema、taxonomy、证据和规则一致性再次校验。
- 增加模型健康、分析、任务查询 API，并在 Web Inspector 显示建议、证据、运行版本和错误状态。
- 初期所有 AI 建议进入 REVIEW；接受动作仍由用户明确执行，AI 不拥有文件系统写权限。

退出条件：

- rules-only 模式可独立启动、扫描和审核。
- 模型停机、超时、无效 JSON、进程重启和重复请求均不阻塞扫描或静默丢任务。
- 同一 input fingerprint 不重复产生并发任务；每次运行可追溯到模型、runtime、prompt、taxonomy 和 schema 版本。
- AI 建议不能直接复制、移动、改名、覆盖、删除或接受任何文件。

### Phase 3：AI 生产验收与 Inbox 集成

内容：

- 从现有 catalog 建立 150–200 个经人工确认的分层 gold set；遮蔽答案路径，并覆盖 Unknown、低置信、冲突与图像证据。
- 对 prompt、图像预处理、runtime 和量化进行固定版本验收；挑战模型不进入关键路径，只有独立 benchmark 通过后才可提议替换。
- 将 AI 触发接入 Inbox：仅限 Unknown、规则低置信、规则冲突、像素证据或用户主动重分析。
- 完成连续两小时队列、重启恢复、模型离线和资源上限测试。

退出条件：

- 后端接纳的结构化结果合法率 100%，已知 modality 准确率不低于 95%，Unknown/冲突召回不低于 95%。
- AI 任务 p95 不超过 30 秒，峰值 VRAM 不超过 90%，两小时测试无崩溃、静默丢任务或泄漏性增长。
- `auto_accept_enabled=false` 保持生效，错误自动接受数为零。
- 模型与 runtime lock 可复现；关闭模型后应用仍以 rules-only 正常工作。
...
- 真正跨页多选、文件级选择、选择快照、预览、后台任务和版本化 manifest。
- Folder bundle、ZIP64 和 manifest-only；导出前后重新验证 SHA-256，原子提交且绝不覆盖。

退出条件：

- 精确选择 18 个 asset 后，manifest 恰有 18 条。
- 每个导出文件哈希与 manifest 一致。
- 缺失、STALE、篡改、重名、重复、预览后 catalog 变化和大文件场景有测试。

### Phase 5：Obsidian Markdown 投影

内容：

- `academic-notes` Vault 骨架。
- outbox、投影器、模板、冲突检测、reconcile。
- Dataset/Project/Sample/Experiment/Method/Paper note 与 `.base` 看板。

退出条件：

- 外部拖入文件后，Obsidian 记录自动出现。
- Obsidian 关闭时仍可入库，重开后记录可见。
- 更新分类不会覆盖 Research notes，Graph 形成真实内部链接边。

### Phase 6：Bridge 与受控双向同步
...
- 用户笔记不被覆盖，受保护字段无法被插件静默修改。
- 两端同时修改共享字段时产生可恢复冲突。

### Phase 7：Web 整合与 Windows 发布

内容：

- 完整新导航、任务中心、模型、导出和集成状态。
- Inspector tabs、响应式改版、键盘和无障碍验收。
- 可复现构建、安装/升级、备份恢复、运行路径显示和发布说明。

退出条件：

- 桌面和移动端无水平溢出。
- 搜索、分页、多选、审核、AI、同步、导出核心路径通过浏览器 QA。
- 无控制台错误和失败请求；Windows 安装版通过日常使用、重启和恢复演练。
...
> 请从 `C:\Research Data\DEVELOPMENT\academic-vault` 开始工作，完整阅读 `DEVELOPMENT_PLAN.md` 和 `BASELINE_AUDIT.md`。Phase 0、GitHub `main` 与 `baseline-pre-v3` 同步已经完成；不要重复迁移，不要修改 `C:\Research Data\academic-vault` 稳定安装版，也不要删除旧开发副本。本期只做 Windows，不开发 Mac profile、Metal、Mac 安装脚本或跨设备同步。当前从 Phase 1“Windows 本地 AI 可行性闸门”继续：先用隔离 benchmark 证明 Qwen3-VL Q8 在 RTX 5080 上的质量、显存、延迟和稳定性，再进入持久队列与产品集成。raw、catalog、Inbox、Vault、模型、运行时、备份、导出物和真实数据派生样本永远不得进入 GitHub；每次提交和推送都必须运行 `scripts/check-git-data.ps1`。模型只有建议权，所有结果进入 REVIEW，自动接受保持关闭。

## 19. 外部条件与单独批准项

- 用户已确认完整路线和 Qwen3-VL Q8 首选策略。模型与便携 llama.cpp 下载仍须记录精确来源、revision、体积、许可证和 SHA-256；不得执行来源不明的安装脚本或系统级全局安装。
- Q8 验收失败后必须停止并报告；下载或测试 Q4 需要用户另行批准，不得静默降级。
- Obsidian 安装属于后续 Phase 5，开始前单独列出来源、版本、权限和变更范围。
- Mac 支持已明确延期，不应在当前 Windows 开发中下载模型、写安装脚本或增加验收阻塞项。
- 何时启用自动接收必须基于 benchmark 结果单独批准，不能仅凭一个置信度阈值自动开启。

## 20. 当前实施状态（2026-07-15）

### 已完成

- Phase 0：开发目录迁移、独立稳定安装边界、migration runner、路径映射、真实后端分页、无 seed 回退、Git/GitHub 数据排除检查均已完成。
- Phase 1 / Phase 2：锁定的 Windows Qwen3-VL Q8 + llama.cpp provider、严格输出契约、确定性证据、持久 AI 队列、重试/租约/重启恢复、API、Inspector 和 rules-only 降级均已完成。
- Phase 3 的 Inbox 触发策略已完成；AI 仍只有建议权，`auto_accept_enabled=false`，不会直接复制、移动、改名、覆盖、删除或接受文件。
- Phase 3 正式两小时 Q8 队列验收已通过：
  - 实际持续 7,216.422 秒；2,692 / 2,692 个任务完成，剩余 active 为 0，失败运行和失败任务均为 0；
  - p50 2.161 秒，p95 4.101 秒，低于 30 秒门槛；
  - GPU 峰值 12,819 / 16,303 MiB（约 78.6%），低于 90% 门槛；
  - 稳态显存中位数从 12,561 MiB 降至 12,441 MiB，增长 -120 MiB，允许上限为 489 MiB；
  - 完成一次受控 worker 重启恢复，结束时 provider 可用，模型身份保持一致；
  - 保护状态前后 SHA-256 一致，raw 只读，验收输出位于 Git 之外。
- Phase 4：schema/API、不可变 selection token、混合 dataset/asset 选择、当前筛选全选、命名 Collection、Folder/ZIP64/manifest-only worker、写出前后哈希、原子提交、版本化 manifest、持久历史页和移动端浏览器 QA 已完成。
- Phase 5 纯代码基础：schema v6、事务内 integration outbox、租约/退避、knowledge/links 表、冲突安全 Markdown projector 和单次可恢复 worker 已完成；全部只在合成临时 Vault 中测试。

### 仍待完成或需要用户参与

- Gold set 已在 Git 外生成 180 条遮蔽候选，但必须由用户人工确认至少 150 条，才能计算已知 modality 准确率和 Unknown/冲突召回；因此 Phase 3 的“质量门”尚未宣告完成。
- 尚未安装 Obsidian、创建真实 `academic-notes` Vault、启动投影后台服务或写入任何真实笔记。开始这些动作前，仍需单独确认 Obsidian 的官方来源、精确版本、权限和变更范围。
- Phase 5 后续还包括知识实体/关系服务、Bases 看板、reconcile 和真实 Vault 恢复演练；Phase 6 Bridge 与 Phase 7 Windows 发布随后执行。

### 当前回归基线

- 后端：116 passed，1 skipped；跳过项为环境相关测试，不是产品失败。
- 前端：Vite production build 通过；导出专用浏览器 QA 覆盖跨页/文件级选择、快照 Collection、后台导出轮询、历史页和 390 px 移动端，无控制台错误、无失败请求、无文档级水平溢出。
- Git：每次提交与推送均通过 tracked/staged 数据安全检查；raw、catalog、模型、runtime、评估明细、导出物和真实数据派生样本均未进入 GitHub。
